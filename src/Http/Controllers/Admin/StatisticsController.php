...
class StatisticsController extends ZainaController
{
  ...
  private function getPeriodInDays( $time ): Period
  {
    ...
    {
      $period = Period::create( Carbon::yesterday(), Carbon::yesterday()->subDays( 7 )->startOfDay() );
    } else if ( $time == 'month' )
    {
  ...
  public function fetchMostVisitedPages( $period )
  {
    ...
    if ( $period == 'day' )
    {
      $pages = Analytics::fetchMostVisitedPages( Period::days( 0 ), 8 );
    ...
    {
      $pages = Analytics::fetchMostVisitedPages( Period::create( Carbon::yesterday(), Carbon::yesterday()->subDays( 7 )->startOfDay() ), 8 );
    } else if ( $period == 'month' )
    {
      $pages = Analytics::fetchMostVisitedPages( Period::months( 0 ), 8 );
    } else
    {
      $pages = Analytics::fetchMostVisitedPages( Period::months( 1 ), 8 );
    }
